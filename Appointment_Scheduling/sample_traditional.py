...
class originalModel:
    ...
    def solveModel(self):
        m2 = grb.Model()
        x = m2.addVars(self.people_num-1, lb = -float('inf'), vtype = GRB.CONTINUOUS, name = 'varx')
...
if __name__ == "__main__":
    num_sample = 5000
    people_num = 10
    alpha = [0.2] * people_num
    beta = 0
    alpha[-1] += (1+beta)
    # zeta = np.random.randint(10, 20, [people_num, num_sample])

    # zeta = np.random.poisson(10, [people_num, num_sample])

    zeta = np.random.exponential(10, [people_num, num_sample])
