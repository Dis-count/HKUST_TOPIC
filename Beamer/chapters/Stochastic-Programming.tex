...
\section{Seat Planning by Stochastic Programming}
    ...
    \begin{frame}{Method Flow}
      \begin{itemize}
        ...
        \item Transform (SSP) to stochastic programming. 
              Problem (1) $\rightarrow$ Problem (2).
        \item Transform Problem (2) to a restricted benders master problem(RBMP) with generative constraints.
        \item (RBMP) can be solved iteratively.
        \item Obtain seat planning by deterministic model.
      \end{itemize}
    \end{frame}
...
\begin{frame}{Solve The Second Stage Problem}
  Problem (3) is easy to solve with the given $x$ which can be seen by the dual problem:
...
\begin{frame}{Deterministic Formulation}  %开始一张幻灯片
  ...
  \begin{equation}\label{deter_lower}
    \begin{aligned}
    \max \quad & \sum_{i=1}^{M}  \sum_{j= 1}^{N} (n_i- s) x_{ij} \\
    \text {s.t.} \quad & \sum_{j= 1}^{N} x_{ij} \geq s_{i}, \quad i \in \mathcal{M}, \\
    & \sum_{i=1}^{M} n_{i} x_{ij} \leq L_j, j \in \mathcal{N} \\
    & x_{ij} \in \mathbb{Z}_{+}, \quad i \in \mathcal{M}, j \in \mathcal{N}.
    \end{aligned}
  \end{equation}
  \end{columns}  %分栏环境结束
  Generate integral seat planning.
\end{frame}
