...
Generally speaking, our problem is also related to network revenue management problem. The network RM problem can be fully characterized by a dynamic programming (DP) formulation. 

% However, as the state space grows exponentially in the number of knapsacks and the decision set grows exponentially in the number of group arrivals per period, the recursion is computationally intractable for large-scale problems.
