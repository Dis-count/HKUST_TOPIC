...
Remark: Notice that this policy ensure a local optimization. We should add the period-control and loss-control. 

When the period is large, we use group-type control and loss control. When the period is small, we just use group-type control. 

Use loss-control, basically, the initial loss will be close to the optimal loss, we want to get a closer loss when we reach the final seat assignment. When the social distance is 1, accepting one group will incur a loss. Every time we break the original seat to smaller one, we will create another loss.

According to the initial demand, we can obtain the loss. The initial ratio is loss/period. Use current loss and current period to record to update current ratio, if (current ratio +2)/(current period +1) < ratio, accept the group. Then current ratio += 2, total loss +=1. Otherwise, reject the group.
