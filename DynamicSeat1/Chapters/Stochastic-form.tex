...
Further more, $\bar{n}_{i+1} = n_{i+1} - n_{i} = 1, i \in [M-1]$ are integral, so the extreme points are all integral.
\qed
\end{pf}
...
\begin{equation}\label{BD_master2}
  \begin{aligned}
    ...
    \text {s.t.} \quad & \sum_{i=1}^{M} n_{i} x_{ij} \leq L_j, j =1,\ldots, N \\
