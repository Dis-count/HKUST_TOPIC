...
\begin{lem}
  If the optimal solution to relaxation of problem (16) takes the form $(0, \ldots, 0, X_{bj}, \ldots, X_{Mj})$. The optimal solution is $z_1 ,\ldots, z_b =0$, $z_{i} = \frac{\delta(n_i-n_b)}{n_b}, i = b+1, \ldots, M$ and $\beta_j = \frac{n_i - \delta}{n_i}, \forall j$.
\end{lem}

When a group type $i$ arrives, we can calculate $i -\beta_{j} n_i$ for all $j$ and choose $\arg \max_{j} \{i -\beta_{j} n_i\}$ as the row to allocate that group. 

\begin{corollary}
  When the capacity allows, the bid-price policy will always accept the large group. 
\end{corollary}

If the optimal solution to relaxation of problem (16) takes the form $(0, \ldots, 0, X_{bj}, \ldots, X_{Mj})$, then according to Lemma 5 and the complementary slackness property, we know that $z_1, \ldots, z_b = 0$. This implies that $\beta_j \geq \frac{n_i - \delta}{n_i}$ for $i = 1,\ldots, b$. Since $\frac{n_i - \delta}{n_i}$ increases with $i$, we have $\beta_j \geq \frac{n_b - \delta}{n_b}$. Consequently, we obtain $z_{i} \geq n_i - \delta - n_i \frac{n_b - \delta}{n_b} = \frac{\delta(n_i-n_b)}{n_b}$ for $i = b+1, \ldots, M$.

Given that $d$ and $L$ are both no less than zero, the minimum value will be attained when $\beta_j = \frac{n_b - \delta}{n_b}$ for all $j$, and $z_i = \frac{\delta(n_i-n_b)}{n_b}$ for $i = b+1, \ldots, M$.

Then the bid-price decision $i -\beta_{j} n_i = i - \frac{n_b -\delta}{n_b} n_i = \frac{\delta (i-b)}{n_b}$. When $i < b$, $i -\beta_{j} n_i <0$. When $i \geq b$, $i -\beta_{j} n_i \geq 0$.
