...
In this subsection, we discuss the effect of different probabilities on our experimental results. Let $\gamma = p_1 * 1 + p_2 * 2 + p_3 * 3 + p_4 * 4$ denote the number of people each period, where $p_1$, $p_2$, $p_3$, and $p_4$ represent the probabilities of groups with one, two, three, and four people, respectively. The expected number of demands can be calculated as $E(D) = \gamma T$,  

We choose $T = E(D)/\gamma$ such that the supply is near the demand. We then compare the number of people served under different values of $\gamma$.
...
Suppose we accept $D_a$ people with $T$ arrivals, where $D_a = \gamma * T$, and the sum of $D_a$ and $T$ is equal to the total number of seats. In this case, the estimation of the occupancy rate is $\frac{\gamma}{\gamma+1}$. The number of people served is near $\frac{\gamma}{\gamma+1} * 210$ on average (as shown by the red points in the figure).
