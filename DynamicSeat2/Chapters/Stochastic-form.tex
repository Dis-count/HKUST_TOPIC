...
Let $\sum_{j=1}^{N} x_{ij}$ indicate the supply for group type $i$. Denote by $\mathbf{X} = (\sum_{j=1}^{N} x_{1j},\ldots, \sum_{j=1}^{N} x_{Mj})$ the aggregate solution to the linear relaxation of problem \eqref{deter_upper}. Let $x_{ij}^{*}$ denote the optimal solution to the linear relaxation of problem \eqref{deter_upper}.

\begin{lem}\label{sol_relax_deter}
There exists $h$ such that for $i < h$, $x_{ij}^{*} = 0$; for $i > h$, $x_{ij}^{*} = d_{i}$; for $i = h$, $\sum_{j} x_{ij}^{*} = (L - \sum_{i = h+1}^{M} {d_i n_i})/ n_h$. 
\end{lem}

Let $e_{i}$ denote the unit size of the $i$-th element of $\mathbf{X}$. Then the aggregate optimal solution is $x e_{h} + \sum_{i=h+1} ^{M} d_{i} e_{i}$, where $x = (L- \sum_{i = h+1}^{M} {d_i n_i})/ n_h$.
