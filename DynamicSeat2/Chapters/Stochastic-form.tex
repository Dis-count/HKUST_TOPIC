...
To maximize the objective function, we can take $y_i^{+} = 0$. Notice that $y_{i}^{-} \geq 0$, thus the constraints $\sum_{j= 1}^{N} x_{ij} + y_{i}^{-} = d_{i}, i \in \mathcal{M}$ can be rewritten as $\sum_{j= 1}^{N} x_{ij} \leq d_{i}, i \in \mathcal{M}$. That is to say, problem \eqref{one_form} is equivalent to the deterministic model.
...
\begin{lem}\label{feasible_region}
  Let $\mathbb{P} = \{\bm{\alpha} \in \mathbb{R}^{M}|\bm{\alpha}^{\intercal} \mathbf{V} \geq \mathbf{f}^{\intercal}\}$. The feasible region of problem \eqref{BD_sub_dual}, $\mathbb{P}$, is not empty and bounded. In addition, all the extreme points of $\mathbb{P}$ are integral.
\end{lem}
...
According to Lemma \ref{feasible_region}, the dual problem \eqref{BD_sub_dual} has an optimal solution and $z_{\omega}(\mathbf{x})$ is finite. Let $\mathcal{O}$ be the set of all extreme points of $\mathbb{P}$, we have the following proposition.

\begin{prop}\label{coro_1}
...
Let the optimal solution to the relaxation of SSP be $\mathbf{x}^{*}$. Aggregate $\mathbf{x}^{*}$ to the number of each group type, ${s}_{i}^{0} =\sum_{j} x^{*}_{ij}, i \in \mathbf{M}$. Replace the vector $\mathbf{d}$ with $\mathbf{s}^{0}$, we have the following problem, 
