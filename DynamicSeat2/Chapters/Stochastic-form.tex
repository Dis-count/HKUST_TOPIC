...
The optimal solution to this problem with the lower bound will give a better seat planning. The numerical results show that this seat planning has good performances under any stochastic demands, and also shows good results when dealing with the dynamic demands. 
...
\begin{algorithm}[H]
  \caption{Feasible seat planning algorithm}\label{feasible_seat}
    ...
    \item[Step 2.] Obtain the optimal solution, $\mathbf{x}^{1}$, from problem \eqref{deter_upper} by setting the supply $\mathbf{s}^{0}$ as the upper bound. Aggregate $\mathbf{x}^{1}$ to the number of each group type, ${s}_{i}^{1} = \sum_{j} x^{1}_{ij}, i \in \mathbf{M}$.
    
    \item[Step 3.] Obtain the optimal solution, $\mathbf{x}^{2}$, from problem \eqref{deter_lower} by setting the supply $\mathbf{s}^{1}$ as the lower bound. Aggregate $\mathbf{x}^{2}$ to the number of each group type, ${s}_{i}^{2} = \sum_{j} x^{2}_{ij}, i \in \mathbf{M}$.
...
% 找到该行中最小的组别大小记为 i，如果最小的组别恰为最大的，则该行对应于最大模式，结束；否则将最小组别的数量减一，将i+1 组别数量加一

\begin{lem}
  For the seat planning obtained in step 3, the number of unoccupied seats is at most $\delta$ for each row.
\end{lem}
