...
\begin{lem}\label{sol_relax_deter}
  Let $x_{ij}^{*}$ denote the optimal solution to the linear relaxation of problem \eqref{deter_upper}. There exists $h$ such that for $i < h$, $x_{ij}^{*} = 0$; for $i > h$, $\sum_{j} x_{ij}^{*} = d_{i}$; for $i = h$, $\sum_{j} x_{ij}^{*} = (L - \sum_{i = h+1}^{M} {d_i n_i})/ n_h$. 
\end{lem}
