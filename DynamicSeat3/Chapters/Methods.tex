...
For each seat planning, we perform the following calculations. First, we calculate $y_{i+1}^{+}$ for each scenario. Next, we count the number of scenarios in which $y_{i+1, \omega}^{+} > 0$ and separately count the number of scenarios in which $y_{i+1}^{+} = 0$.

To make a decision on whether to accept or reject the group, we calculate the value $i P(y_{i+1, \omega}^{+} > 0) - (i+1) P(y_{i+1, \omega}^{+} = 0)$. If this value is greater than 0, we accept the group. Otherwise, if the value is less than or equal to 0, we reject the group.

However, this approach cannot help us find the specific row to assign the group.
% We still need the breaking tie rule to assign the group to the specific row
