...
Let $|h|$ indicate the number of people that can be assigned with pattern $\bm{h}$, i.e., $|h| = \sum_{i =1}^{M} i h_i$. Then we have $h_i h_j = 0 ~\text{if}~ n_i + n_j \leq n_M$. We also introduce the concept of loss, which is the number of unoccupied seats. Mathematically, the loss is defined as $L- \delta - |h|$, where $L$ denotes the length of the row. The loss provides a measure of the number of seats which cannot be taken due to the implementation of social distancing constraints. By examining the losses associated with different patterns, we can assess the effectiveness of various seat planning configurations with respect to accommodating the desired number of individuals while adhering to social distancing requirements.

% $|h(L)| \geq |h(L+1)| -1$.

\begin{definition}
Consider a feasible pattern $\bm{h} = (h_1, \ldots, h_M)$. 
We call $\bm{h}$ a full pattern if it satisfies the condition $\sum_{i=1}^{M} h_i n_i = L$. We call $\bm{h}$ a largest pattern if $|\bm{h}| \geq |\bm{h}^{\prime}|$ for any $\bm{h}^{\prime}$.
