...
Considering the analysis provided earlier, we find it advantageous to obtain a seat planning that only consists of full or largest patterns.
...
\begin{algorithm}
  ...
  \While{$\beta > 0$}
    ...
    $h_{\min\{k+\beta, M\}} \gets h_{\min\{k+\beta, M\}} + 1$\;}
    {\eIf{$\beta \geq n_{M}$}
    {$q \gets \lfloor\frac{\beta}{n_M}\rfloor$\;
     $\beta \gets \beta - q n_M$\; $h_{M} \gets h_{M} + q$\Comment*[r]{This is a comment}}
    {\eIf{$n_{1} \leq \beta < n_{M}$}
    {$h_{\beta-n_1+1} \gets h_{\beta-n_1+1} + 1$\; $\beta = 0$\;}
    {$\bm{h}$ is the largest}}
    }}
\end{algorithm}
