...
\begin{algorithm}
  ...
  \KwIn{Integer $\mathbf{x}$, $P=\{P_1,P_2,\ldots,P_n\}$}
  \KwOut{$P$}
  \For{$j=1, \ldots, N$}
  {\While{$\beta > 0$}
    {\While{$\beta \geq n_M$}
    {$\beta \gets \beta - n_M$\; $P_{j,M} \gets P_{j,M} + 1$\;}
...
We use $\mathbf{x} \mathbf{1}$ to indicate the supply.
...
\begin{algorithm}
  ...
  \While{$UB - LB > \epsilon$}
    ...
    {Obtain $\alpha_{\omega}^{1}$ from Proposition \ref{optimal_sol_sub_dual}\; $z_{\omega}^{(0)} = (\alpha_{\omega}^{1})^{\intercal}(\mathbf{d}_{\omega}- \mathbf{x}_0 \mathbf{1})$\;
...
From Proposition \ref{one_ep_feasible}, we can set $\bm{\alpha}_{\omega}^0 = \mathbf{0}$ initially. Notice that only contraints are added in each iteration, thus $UB$ is decreasing monotone over iterations and $LB$ is increasing monotone over iterations. Then we can use $UB - LB < \epsilon$ to terminate the algorithm.
