...
Because patterns are independent, for any given seat planning, we can convert the patterns row by row.

Considering the analysis provided earlier, we find it advantageous to obtain a seat planning that primarily consists of full or largest patterns. Given a specific pattern, we have the ability to convert it into a large or full pattern while ensuring that the original group type requirements are met. When converting a pattern to a full pattern, we strive to generate a pattern that minimizes the loss. Since patterns are independent of each other, we can convert them row by row within a given seat planning. This approach allows us to optimize the seat planning by maximizing the utilization of available seats.

% By adopting this strategy of converting patterns row by row and prioritizing full or largest patterns, we can enhance the effectiveness of the seat planning process, optimize seat utilization, and minimize any potential losses.
...
\begin{algorithm}
  ...
  \KwIn{Integral $\mathbf{x}$, $P=\{P_1,P_2,\ldots,P_n\}$}
  \KwOut{$P$}
  \For{$j=1, \ldots, N$}
  {\While{$\beta > 0$}
    {\While{$\beta \geq n_M$}
    {$\beta \gets \beta - n_M$\; $P_{jM} \gets P_{jM} + 1$\;}
    \eIf{$n_{1} \leq \beta < n_{M}$}
    {$P_{j,\beta-n_1+1} \gets P_{j,\beta-n_1+1} + 1$\; $\beta = 0$\;}
    {$i^{0} = \min_{k}\{P_{jk} \neq 0\}$\;
    \eIf{$i^{0} \neq M$}
    {$\beta \gets \beta - \max\{1, M - i^{0}\}$\; $P_{j,i^{0}} \gets P_{j,i^{0}} - 1$\;
    $P_{j,\min\{i^{0}+\beta, M\}} \gets P_{j,\min\{i^{0}+\beta, M\}} + 1$\;}
    {$\beta = 0$}}}
    }
\end{algorithm}
...
\begin{algorithm}
  ...
  \KwIn{Initial problem \eqref{BD_master3} with $\alpha_{\omega} = 0, \forall \omega$, $LB = 0$, $UB = \infty$, $\epsilon$.}
  \KwOut{$\mathbf{x}_{0}$}
  \While{$UB - LB > \epsilon$}
    {Obtain $(\mathbf{x}_{0}, \mathbf{z}^{0})$ from problem \eqref{BD_master3}\;
    $UB = c^{\intercal} \mathbf{x}_0 + \sum_{\omega \in \Omega} p_{\omega} z_{\omega}^{0}$\;
    \For{$\omega= 1, \ldots, |\Omega|$}
    {Obtain $\alpha_{\omega}^{1}$ from Proposition 6\; $z_{\omega}^{(0)} = (\alpha_{\omega}^{1})^{\intercal}(\mathbf{d}_{\omega}- \mathbf{x}_0 \mathbf{1})$\;
    \If{$z_{\omega}^{(0)} < z_{\omega}^{0}$}
    {Add one new constraint, $(\alpha_{\omega}^{1})^{\intercal}(\mathbf{d}_{\omega}- \mathbf{x} \mathbf{1}) \geq z_{\omega}$, to problem \eqref{BD_master3}\;}
    }
    {$LB \gets c^{\intercal} \mathbf{x}_{0} + \sum_{\omega \in \Omega} p_{\omega} z_{\omega}^{(0)} $\;}
    }
\end{algorithm}

From the Proposition \ref{one_ep_feasible}, we can set $\bm{\alpha}_{\omega}^0 = \mathbf{0}$ initially. Notice that only contraints are added in each iteration, thus $LB$ and $UB$ are both monotone. Then we can use $UB - LB < \epsilon$ to terminate the algorithm.
...
\begin{algorithm}
  ...
  \KwOut{$\mathbf{x}, P$}
    ...
    {${X}_{i}^{0} \gets \sum_{j} x^{*}_{ij}, i \in \mathbf{M}$\;}
    {Obtain the optimal solution, $\mathbf{x}^{1}$, and the corresponding pattern, $P$, from problem \eqref{deter_upper1}\;}
    {Construct the full or largest patterns by algorithm \ref{construction} with $\mathbf{x}^{1}$ and $P$\;}
\end{algorithm}
