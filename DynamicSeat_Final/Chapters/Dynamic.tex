...
% Similarly, let the corresponding supply be denoted as $[X_1, \ldots, X_M]$, where $X_i = \sum_{j} x_{ij}$ for all $i$. When a group of type $i$ arrives, if $X_i > 0$, we accept the group directly and assign them the seats originally planned for group type $i$. When $X_i = 0$, we determine the group type that can accommodate the arriving group. Then, we choose a specific row according to the group type and the break-tie rule, then make the final decision by evaluating the values obtained from the relaxed SSP. Afterward, we consider certain conditions to determine when to regenerate the seat planning, in order to avoid regenerating it every period. This iterative process continues for the next incoming group, ensuring that seat assignments are made in real-time based on the current seat planning, while minimizing unnecessary regeneration of the seat planning.

% When $X_i =0$, the seat assignment process involves two main steps. First, we determine the appropriate group type used to accommodated the arriving group. The second step is to choose a specific row according to the group type and make the final decision by evaluating stochastic programming. For the whole periods, we regenerate the seat planning under certain conditions to optimize computational efficiency.
...
SSP$(\mathbf{L}_{a}^{t}, \Omega^{t})$

Suppose a group type $i$ arrives at period $t$. The set of scenarios at period $t$ is denoted as $\Omega^{t}$. The available supply at period $t$ before making the decision is $\mathbf{L}_{r}^{t} = (L_1^{t}, \ldots, L_N^{t})$. The VoR is the value of relaxed SSP with the scenario set $\Omega^{t}$ and the capacity $\mathbf{L}_{r}^{t}$ when we reject group type $i$ at period $t$, denoted as SSP$(\mathbf{L}_{r}^{t}, \Omega^{t})$. If we plan to accept group type $i$ in row $j$, we need to assign seats from row $j$ to group type $i$. Let $\mathbf{L}_{a}^{t}= (L_1^{t}, \ldots, L_j^{t}-n_{i}, \ldots, L_N^{t})$, then the VoA is calculated by SSP$(\mathbf{L}_{a}^{t}, \Omega^{t})$.
...
% Based on the supply, we can determine whether to accept an incoming group. When a group arrives, if there is sufficient supply available for an arriving group, we will accept the group and choose the group type accordingly. 
...
\begin{algorithm}[H]
  ...
  { Observe group type $i$\;
    ...
    \If{$i = M$ and $X_{M} =0$}
    {Regenerate $\bm{H}$ from Algorithm \ref{seat_construction}\; Update the corresponding $\bm{X}$\Comment*[r]{Regenerate the seat planning when the supply of the largest group type is 0}}
    }
    {Calculate $d^{t}(i, j^{*})$\;
    \eIf{$d^{t}(i, j^{*}) >0 $}
...
\begin{algorithm}[H]
  ...
  {Observe group type $i$\;
  \If{$i = M$ and $X_{M} =0$}
   {Generate $\bm{H}$ from Algorithm \ref{seat_construction}\; Update the corresponding $\bm{X}$\Comment*[r]{Regenerate the seat planning when the supply of the largest group type is 0}}
    Generate seat planning with scenarios\;
    \eIf{$X_{i} > 0$}
    {Find row $k$ such that $H_{ki} >0$ according to tie-breaking rule\; Accept group type $i$ in row $k$, $L_{k} \gets L_{k} -n_{i}$\; $H_{ki} \gets H_{ki} -1$, $X_{i} \gets X_{i} -1$\Comment*[r]{Accept group type $i$ when the supply is sufficient}}
    {Calculate $d^{t}(i, j^{*})$\;
    \eIf{$d^{t}(i, j^{*}) >0 $}
