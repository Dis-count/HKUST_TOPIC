...
In this section, we address the deterministic problem by integrating social distancing measures into seat planning. We then introduce the concept of a full or largest pattern. For seat planning that does not utilize all available seats, we propose a method to enhance it by incorporating full or largest patterns. Finally, we showcase the dynamic seat assignment problem.
...
Let $|\bm{h}|$ indicate the number of people that can be assigned according to pattern $\bm{h}$, i.e., $|\bm{h}| = \sum_{i =1}^{M} i h_i$. The size of $\bm{h}$ provides a measure of the largest number of seats which can be taken due to the implementation of social distancing constraints. By examining $|\bm{h}|$ associated with different patterns, we can assess the effectiveness of various seat planning configurations with respect to accommodating the desired number of individuals while adhering to social distancing requirements.
...
After the conversion, $L = L^{0} + 1 =11$, $n_i = i + 1$ for $i = 1, 2, 3, 4$. For the first group, we use a group of three seats to indicate the original group of two and one seat used as the social distancing. We do the same procedure for the other groups. Then, the row can be represented by $\bm{h} = (2,1,1,0)$. The largest number of people that can be accommodated is $|\bm{h}| = 7$.
\end{example}
...
The largest pattern $\bm{h}$ is unique and full when $r = 0$, indicating that only one pattern can accommodate the maximum number of people. On the other hand, if $r > \delta$, the largest pattern $\bm{h}$ is full, as it utilizes the available space up to the social distancing requirement.
...
\begin{itemize}
\item For $i = 1,\ldots, \tilde{i}-1$, $x_{ij}^{*} = 0$ for all rows, indicating that no group type $i$ are assigned to any rows before index $\tilde{i}$.
...
The seat planning obtained from problem \eqref{deter_upper} may not utilize all available seats, as it depends on the given demand. To improve a given seat planning and utilize all seats, we aim to generate a new seat planning composed of full or largest patterns. Since each row is dependent, we can generate the patterns row by row. For a specific pattern, we can convert it into a largest or full pattern while ensuring that the original group type requirements are met. When multiple full patterns are possible, our objective is to generate the pattern with the largest size. Mathematically, for any feasible pattern $\bm{h} = (h_1, \ldots, h_M)$, we seek to find a pattern $\bm{h}^{'} = (h_1^{'}, \ldots, h_M^{'})$ that satisfies the following programming:

\begin{equation}\label{gene_largest}
  \begin{aligned}
  \max \quad & |\bm{h}^{'}| \\
  s.t. \quad & h_M^{'} \geq h_M \\
  & h_{M-1}^{'} + h_{M}^{'} \geq h_{M-1} + h_{M} \\
  & \cdots \\
  & h_1^{'} + \ldots + h_M^{'} \geq h_1 + \ldots + h_M
  \end{aligned}
\end{equation}

% maximizes $|\bm{h}^{'}|$ while satisfying the following constraints: $h_M^{'} \geq h_M$, $h_{M-1}^{'} + h_{M}^{'} \geq h_{M-1} + h_{M}$, $\cdots$, $h_1^{'} + \ldots + h_M^{'} \geq h_1 + \ldots + h_M$. 
...
Algorithm \ref{construction} can generate an optimal solution to problem \eqref{gene_largest} with the give feasible pattern $\bm{h}$.
\end{prop}


Since patterns are independent of each other, we can employ Algorithm \ref{construction} row by row within a given seat planning. This enables us to optimize the seat planning by maximizing the utilization of available seats.
