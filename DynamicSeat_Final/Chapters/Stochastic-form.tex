...
\begin{equation}\label{deter_upper1}
  \left\{\max \sum_{j=1}^{N} \sum_{i=1}^{M}(n_i -\delta) x_{ij}: \sum_{i = 1}^{M} n_i x_{ij} \leq L_{j}, j \in \mathcal{N}; \sum_{j =1}^{N} x_{ij} \leq {X}^{*}_{i}, i \in \mathcal{M}; x_{ij} \in \mathbb{N},i \in \mathcal{M}, j \in \mathcal{N} \right\}
\end{equation}

Then solve the resulting problem \eqref{deter_upper1} to obtain the optimal solution, $\mathbf{\tilde{x}}$, which represents a feasible seat planning.
...
\begin{algorithm}
  ...
  % \KwOut{$\mathbf{x}, \bm{H}$}
    ...
    {Construct the full or largest patterns by problem \eqref{improve_seat} with $\tilde{\mathbf{x}}$ and $\bm{H}$\;}
\end{algorithm}
