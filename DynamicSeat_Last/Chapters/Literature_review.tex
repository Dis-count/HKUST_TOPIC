...
Prior work has examined group arrivals in the contexts such as: in hotel revenue management \citep{bitran1995application, goldman2002models}, where customers request multi-day stays but room assignments can be deferred until check-in; in high-speed train ticket sales \citet{zhu2023assign}, where each booking must be assigned to a specific seat for the entire multi-leg journey in real time. Both scenarios involve assignment challenges, but with key differences: hotels allow flexible room allocation, while train seating requires immediate assignment. Our SADR problem aligns more closely with the latter - requiring real-time group assignment - but differs in that requests specify only group size, without fixed start dates (for hotels) or boarding stops (for trains).
 
 
%  we only have the assignment constraints (e.g., ensuring group members are seated together). but only with the request of group size, don't specify which day to start to stay in the hotel or which stop to start the journey for the train.


% While traditional RM focuses on single-unit demand, group-based RM introduces challenges in capacity allocation. 

% Most of the work focuses on individual requests and booking decisions. 


% Prior work has examined group arrivals in the context of hotel revenue management such as: multi-days stays \citep{bitran1995application, goldman2002models}. However, most existing models simplify group requests by treating them as single aggregated demands or assume independent allocations, neglecting assignment constraints (e.g., ensuring group members are seated together).


% Airlines: Families or tour groups requiring multiple adjacent seats, they usually make the pre-allocation for the groups based on the historical data and demand forcast or provide the block booking 


% they focus on the different types, no-show, overbooking, cancellations
...
% And the problem becomes more complex due to assignment constraints (e.g., ensuring group members are seated together or assigned to the same hotel room type).
...
% group price / discount/ allocation /decision 

% individual /multiple states 


% The assign-to-seat feature introduced by \citet{zhu2023assign} further emphasizes the significance of seat assignment. 

% This approach tackles the challenge of selling high-speed train tickets, where each request must be assigned to a specific seat for the entire journey. However, this paper focuses on individual passengers rather than groups, which sets it apart from our research.
