...
% Formulate it by IP to develop seat planning.

% For the seat planning that does not utilize all available seats, we propose to improve the seat planning.
...
% Let $f(M, L)$ indicate the size of any largest pattern under M and L. $f(M, L)$ does not decrease in $M$. Since any largest pattern $\bm{h}$ under M is a feasible pattern under $(M+1)$, thus, $f(M, L) \leq $f(M+1, L)$

% Similarly, $f(M, L)$ does not decrease in $L$. Since any largest pattern $\bm{h}$ under L is a feasible pattern under $L+1$, thus, $f(M, L) \leq $f(M, L+1)$.
