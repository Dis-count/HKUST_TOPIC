...
% Similarly, $\phi(M, L; \delta)$ does not decrease in $L$. Since any largest pattern $\bm{h}$ under $L$ is a feasible pattern under $L+1$, thus, $\phi(M, L) \leq \phi(M, L+1)$.

% $\phi(M, L; \delta)$ does not decrease in $M$.

% Consequently, when $M$ increases while $\bm{L}$ remains unchanged, the maximum achievable occupancy rate does not decrease.
