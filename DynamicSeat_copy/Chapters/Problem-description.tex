...
\begin{example}
Consider the given values: $\delta = 1$, $L^{0} = 10$, and $M = 4$. By adding one seat to each group and the original row, we can realize the conversion, as shown in the follwoing figure.
...
Given a specific pattern, we can convert it into a largest or full pattern while ensuring that the original group type requirements are met. When multiple full patterns are possible, our objective is to generate the pattern with the largest size. Mathematically, for any pattern $\bm{h} = (h_1, \ldots, h_M)$, we seek to find a pattern $\bm{h}^{'} = (h_1^{'}, \ldots, h_M^{'})$ that maximizes $|\bm{h}^{'}|$ while satisfying the following constraints: $h_M^{'} \geq h_M$, $h_{M-1}^{'} + h_{M}^{'} \geq h_{M-1} + h_{M}$, $\cdots$, $h_1^{'} + \ldots + h_M^{'} \geq h_1 + \ldots + h_M$. By finding such a pattern $\bm{h}^{'}$, we can ensure that the converted pattern meets or exceeds the requirements of the original group types.

% In other words, we want to find a pattern $\bm{h}^{'}$ where each element $h_i^{'}$ is greater than or equal to the corresponding element $h_i$ in $\bm{h}$, and the cumulative sums of the elements in $\bm{h}^{'}$ are greater than or equal to the cumulative sums of the elements in $\bm{h}$.

Now, we demonstrate the specific allocation scheme.
Let $\beta_{j}= L_{j} - \sum_{i} n_{i} x_{ij}$. If row $j$ is not the largest or full, then $\beta_{j} > 0$. Our goal is to allocate the remaining unoccupied seats in row $j$ in a way that prioritizes making the planned groups as large as possible. If there are still unoccupied seats available, we aim to allocate them to the planned largest groups as much as possible. Find the smallest group type in the pattern denoted as $k$. If $k = M$, it means that this row corresponds to a largest pattern. If $k \neq M$, we reduce the number of group type $k$ by one and increase the number of group type $\min \{(k+\beta_{j}), M\}$ by one, the number of unoccupied seats will be reduced correspondingly.
...
By following these steps and always prioritizing the largest group type for seat planning, we can achieve the largest or full pattern. This approach guarantees efficient seat allocation, maximizing the utilization of available seats while still accommodating the original groups' requirements.
