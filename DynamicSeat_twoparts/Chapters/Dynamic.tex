...
% 1. To facilitate Dynamic Programming (DP), 

% determine the value of $\tilde{i}$ such that we accept group types greater than $\tilde{i}$.
...
\begin{algorithm}[H]
  ...
  \For{$t =1, \ldots, T$}{
    {Observe group type $k$\;}
    \If{$k$ is in $\bm{H}$}
    {Place the group in the corresponding row\; \textbf{break}\;}

    \eIf{$V^{t+1}(l^{t}) \leq V^{t+1}(l^{t}-n_{k}) + k$}
    {\For{$j =1, \ldots, N$}
      {Solve problem \eqref{dp_pattern} for row $j$, obtain the pattern $\bm{h}^{j}$, update $\bm{H}$\;
    \If{The pattern $\bm{h}^{j}$ is full or largest}
    {Place the group in row $j$\; \textbf{break}\;}}    
    }{Reject the group\;}}
\end{algorithm}