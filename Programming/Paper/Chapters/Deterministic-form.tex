...
\begin{table}[ht]
  \begin{tabular}{l|l|l|l|l}
  \hline
  \# samples & T & probabilities & \# rows & relative performance(\%) compared to the optimal \\
...
5: The maximal people from the offline sequence.
...
Then we can use DP to decide accept or reject in each period.
...
Group type, $k$, has an integer weight $w_k$, a value $b_k$, and an arrival rate $\lambda_{k}(t)$.

Consider arrivals as a nonhomogeneous Poisson process over a continuous time horizon $[0, T]$ to a knapsack with capacity $W$.
...
$$
\begin{aligned}
f(y, t) &= \int_0^t \lambda(s) e^{-\int_s^t \lambda(\tau) \mathrm{d} \tau} \cdot \sum_{k=1}^K p_k(s) \max \left\{b_k+f\left(y-w_k, s\right), f(y, s)\right\} \mathrm{d} s \\
 &= \int_0^t \lambda(s) e^{-(\Lambda(t)-\Lambda(s))} \cdot \sum_{k=1}^K p_k(s) \max \left\{b_k+f\left(y-w_k, s\right), f(y, s)\right\} \mathrm{d} s,
\end{aligned}
$$
...
$$
\begin{aligned}
f^{\prime}(y, t)= & \lambda(t) \sum_{k=1}^K p_k(t) \max \{b_k+f\left(y-w_k, t\right),f(y, t)\}-\lambda f(y, t) \\
= & \lambda(t) \sum_{k=1}^K p_k(t) \max \{0, b_k+f\left(y-w_k, t\right) -f(y, t)\} \\
= & \sum_{k=1}^{K} \lambda_{k}(t) \max \{0, b_k+f\left(y-w_k, t\right) -f(y, t)\}
\end{aligned}
$$

If the $\lambda_{k}(t) > 0$ for all $k$ and $t$, then $f(y,t)$ is strictly monotone increasing in $t$ for $y \geq \min_{k} w_k$.
