...
if __name__ == "__main__":
    ...
    for num_period in period_range:
        ...
        for j in range(count):
            sequence, newx4 = a_1.random_generate()
            sequence1 = [i-sd for i in sequence]
            _, newx40 = a_2.random_generate()
            f = a_0.offline(sequence1)  # optimal result
            optimal = np.dot(multi, f)

            # d = a_instance.offline(sequence)
            g0 = a_2.method_new(sequence, newx40, roll_width)
            sto_2 += np.dot(multi, g0)

            g = a_1.method_new(sequence, newx4, roll_width)
    ...
    plt.plot(t_value, occup_1, 'y*', label='With 2 social distancing')
