...
if __name__ == "__main__":
    ...
    for p in prop:
        ...
        for j in range(count):
            ...
            d = a_instance.method_IP_group(sequence, newx3, roll_width)

            f = a_instance.offline(sequence)  # optimal result
            optimal = np.dot(multi, f)

            ratio1 += np.dot(multi, a) / optimal   # sto-planning
            ratio2 += np.dot(multi, d) / optimal   # bid-price
            accept_people += optimal

        my_file.write('Sto: %.2f ;' % (ratio1/count*100))
        my_file.write('book_group: %.2f ;' % (ratio2/count*100))
