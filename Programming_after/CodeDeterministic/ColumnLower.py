...
# seat assignment with lower bound demand
...
def master_problem(column, vtype):
    ...
    m.addConstr(lhs = column @ x >= lower_demand_array)
...
def knapsack_subproblem(kk):
    m = grb.Model()
    length = int((kk.shape[0]-1)/2)
    x = m.addMVar(shape = length, lb=0, vtype=GRB.INTEGER)
    m.addConstr(lhs = demand_width_array @ x <= roll_width)
    lambda_kk = kk[0:length]
    mu_kk = kk[length:-1]
    m.setObjective((value_array - lambda_kk - mu_kk) @ x-kk[-1], GRB.MAXIMIZE)
    m.setParam('OutputFlag', 0)
    m.optimize()
...
def IP_formulation(n, given_lines):
    ...
    m.addConstrs(grb.quicksum(x[i,j] for j in range(given_lines)) >= lower_demand_array[i] for i in range(n))
...
if __name__ == '__main__':
    ...
    demand_width_array = np.array([2, 3, 4, 6, 8]) # [1,3,5,6] people in each group
    value_array = demand_width_array -1
    initial_cut_pattern = np.diag(np.floor(roll_width / demand_width_array))
    demand_number_array = np.array([10, 17, 20, 10, 15])
    lower_demand_array = np.array([1, 1, 1, 1, 1])
    n = len(demand_width_array)
    given_lines = 11

    IP_formulation(n, given_lines)
    columnGeneration()
