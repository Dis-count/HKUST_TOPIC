...
class CompareMethod_new:
                ...
                for j in range(1, T + 1):
                    ...
                    for k in range(option):
                        if (i - self.demand_width_array[k]) >= 0:
                            everyvalue2 = value[i2][i - self.demand_width_array[k]
                                                    ][j - 1] + self.value_array[k]

                        if (i2 - self.demand_width_array[k]) >= 0:
                            everyvalue1 = value[i2 - self.demand_width_array[k]
    ...
    def main_dy(self, sequence, value):
        decision_list = [0] * self.num_period
        cur_roll_width = copy.deepcopy(self.roll_width)
        for num, i in enumerate(sequence):
...
if __name__ == "__main__":
    ...
    num_sample = 1000
    s = 1
    a = CompareMethod_new(roll_width, given_lines, I, probab, num_period, num_sample, s)
    sequence, ini_demand, ini_demand3, newx3, newx4 = a.random_generate()

    value = a.dynamic2(220, 220, 71)
    aaa = np.array(value)
    np.save('a.npy', aaa)
