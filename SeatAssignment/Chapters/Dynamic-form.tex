...
For reservations without seat selection, the seat assignment will not be made immediately, decision-maker only needs to reject or accept each request during the making-reservation stage. After the reservation deadline, the seat layout information will be informed to the customers before admission. For example, singing concert venues have more seats and higher demand for tickets, organizers usually do not take the approach of determining the seats at the time of booking, but not provide seat information, and then inform customers of the seat information after the overall seat layout is determined.
...
We can use stochastic information to generate scenarios integrated from arrival sequences. Suppose there are $T$ independent periods; at most, one group will arrive in each period.
...
\begin{algorithm}[H]\label{general_method}
  ...
  \begin{description}
    \item[Step 1.] Obtain a linear solution from stochastic programming \eqref{BD_master2}.
