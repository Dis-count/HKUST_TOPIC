...
Let $c = p_1 * 1 + p_2 * 2 + p_3 * 3 + p_4 * 4$. Notice that different $c$ need different $T$ such that the supply is close to the demand. Different $T$ has little effect on the number of people served when using M6 because it is based on first-come-first-served. 

Thus, we use M6 to compare the number of people served under different values of $c$.

Sample $p_1, p_2, p_3$ from $0.05$ to $0.95$ with increment of $0.5$. The number of rows is 10, the number of seats for each row is 21 (including one dummy seat). The number of periods is $200$. The number of people served with respect to the value of $c$ is shown below:
...
If the largest pattern is assigned for each row, then the occupancy rate is $\frac{16}{20} = \frac{4}{5}$. The maximal number of people can be served is $200 * \frac{4}{5} =160$. 
