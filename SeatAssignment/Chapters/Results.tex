...
The number of people served is near $\frac{c}{c+1} * 210$ for most instances. $\frac{c}{c+1}$ is a rough estimation of occupancy rate when supply is close to demand. (Suppose $E(D) + T$ = the total number of seats, we accept $E(D)$ people with $T$ arrivals. Then the occupancy rate is $\frac{c * T}{(c+1) * T}= \frac{c}{c+1}$).
