...
Now suppose the demand is stochastic, following some unknown distribution. The stochastic information can be obtained from scenarios through historical data. As mentioned above, the objective is to give a seat assignment that can place as many groups as possible. Then we propose a two-stage scenario-based stochastic programming.

Consider the decision maker who has to assign the seats in two consecutive stages. The seat assignment and demand take place in the first and second stages, respectively. The first stage involves the initial seat assignment denoted by decision variables $\mathbf{x}\in \mathbb{Z}_{+}^{m \times N}$. These variables are scenario-independent and each component $x_{i,j}$ stands for the number of group type $i$ in row $j$.
...
% The stochastic information about demands is obtained. 
%  Write it specifically. what information.

In the second stage, the second-stage scenario-dependent decision variable, $\mathbf{y}$, is to be  chosen. It includes two vectors of decisions, $\mathbf{y}^{+} \in \mathbb{Z}_{+}^{m \times S}$ and $\mathbf{y}^{-} \in \mathbb{Z}_{+}^{m \times S}$. Each component of $\mathbf{y}^{+}$, $y_{i \omega}^{+}$, represents the number of holding $i$-th groups, which is positive when the supply is larger than the actual demand, zero otherwise. Similarly, $y_{i \omega}^{-}$ represents the number of short groups, which is positive when the supply is less than the actual demand and zero otherwise.
...
As we can find, this deterministic equivalent form is a large-scale problem even if the number of possible scenarios $\Omega$ is moderate. However, the structured constraints allow us to simplify the problem by using a decomposition approach.

Let $c{'}\mathbf{x} = \sum_{j =1}^{N} \sum_{i=1}^m i x_{ij}$, $f{'}y_{\omega} = -\sum_{i=1}^{m} y_{i \omega}^{+}$. Then the formulation \eqref{sto_form} can be expressed as below,
...
\begin{equation}\label{BD_sub}
  \begin{aligned}
    z_{\omega}(\mathbf{x}) := \max \quad & f{'} \mathbf{y}_{\omega} \\
...
\section{Solve The Two-stage Problem}\label{solve_by_benders}

% In section \ref{second_stage}, we obtain a closed-form solution to the second-stage problem.
...
If we need to consider the lower bound of demand, which represents the groups that must be allocated seats, we can add a new constraint $\sum_{j =1}^{N} x_{ij} \geq d_{i}^{l}, i=1,\ldots,m$ in problem \eqref{deter_upper}, $d_{i}^{l}$ is the number of group type $i$ we have placed.
