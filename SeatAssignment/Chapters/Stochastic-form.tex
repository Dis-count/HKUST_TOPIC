...
As mentioned earlier, the objective is to give a seat assignment that can accept as many groups as possible. We develop stochastic programming by the scenarios modeling demand uncertainty.

\subsection{Two-stage Scenario-based Stochastic Programming}

Consider the decision maker who has to act in two consecutive stages. The first stage involves the initial seat assignment denoted by decision vector $\mathbf{x}$. In the second stage, some new information about demands is obtained, then a new vector $\mathbf{y}$ of decisions is to be chosen.

$\mathbf{x} \in \mathbb{Z}_{+}^{n}$ is the vector of first-stage scenario-independent decision variables, and each component $x_k$ stands for the pattern $k$.
...
Let $p_{\omega}$ denote the probability of any scenario $\omega$, which we assume to be positive. We use the exact definition above, the size of group type $i$, $s_i$.

% The assignment will be determined before the realization of the random demand, here-and-now policy.

Considering that the smaller group type can take the seats assigned to some group type, we assume that the holding group type $i$ can be utilized by the smaller group type $j = i-1$. Then we have the following scenario-based optimization problem:
...
\subsection{Solve The Two-stage Problem by Benders Decomposition}

\subsubsection{Solve The Second Stage Problem}
...
Now go back to consider the optimization of $x$, we have the problem:
...
To solve this problem, we should only consider that $x$ for which $z_{\omega}(x)$ are all finite. Notice that the feasible region of the dual of problem \eqref{BD_sub} does not depend on $x$. We can form its dual problem, which is 
...
\begin{corollary}
  Only the optimality cuts, $\alpha{'}(\mathbf{d}_{\omega}- \mathbf{x} \mathbf{1}) \geq z_{\omega}$, will be included in the following decomposition method.
\end{corollary}
...
We can use the forward method, calculating from $\alpha_{1 \omega}$ to $\alpha_{m \omega}$, to obtain the value of $\alpha_{\omega}$ rather than solving linear programming.
...
\subsubsection{Delayed Constraint Generation}

Let $z_{\omega}$ be the lower bound of $z_{\omega}(x)$ such that $(\alpha^{k}){'}(\mathbf{d}_{\omega}- \mathbf{x} \mathbf{1}) \geq z_{\omega}, \alpha^k \in \mathcal{O}$, which is the optimality cut.
...
The corresponding constraints can be removed when the lower or upper bound is not needed. The solver can quickly solve this deterministic formulation with lower or upper bound because of the monotone ratio.
...
\begin{algorithm}[H]\label{near-optimal}
  \caption{Near-optimal seat assignment algorithm}
    ...
    \item[Step 3.] Set the supply as the upper bound of demand in problem \eqref{deter_lower_upper}. The lower bound is not included.
    
    \item[Step 4.] Aggregate the solution from problem \eqref{deter_lower_upper} to a supply for each group type.

    \item[Step 5.] Obtain the near-optimal assignment algorithm from \eqref{deter_lower_upper} by setting the supply as the lower bound.
   \end{description}
  \end{algorithm}

\begin{remark}
  Step 3 can give a feasible integer supply. Then problem \eqref{deter_lower_upper} with this supply as the lower bound can always give an integer solution.  
