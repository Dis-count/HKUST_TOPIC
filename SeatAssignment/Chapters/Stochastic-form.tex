...
Consider the decision maker who has to act in two consecutive stages. The first stage involves the initial seat assignment denoted by decision vector $\mathbf{x}$. At the second stage, some new information about demands is obtained, then a new vector $\mathbf{y}$ of decisions is to be chosen.
...
Let $p_{\omega}$ denote the probability of any scenario $\omega$, which we assume to be positive. We use the same definition as above, the size of group type $i$, $s_i$.
...
The optimal value of the problem \eqref{BD_sub}, $z_{\omega}(x)$, is finite and this value will be attained at extreme points of the set $P$. Thus, we have $z_{\omega}(x) = \min_{\alpha^j \in \mathcal{O}} (\alpha^{k}){'}(\mathbf{d}_{\omega}- \mathbf{x} \mathbf{1})$. 
...
The stochastic model only gives a fractional solution; its objective is to obtain the maximal number of people that can be served, not the optimal seat assignment. It will not give an appropriate solution when the number of arriving people of the scenarios is way less than the number of total seats because it does not utilize all the empty seats.
...
When the lower or upper bound is not needed, the corresponding constraints can be removed. This deterministic formulation with bounds can be solved quickly because of the monotone ratio.
...
\begin{remark}
  Step 3 can give a feasible integer supply. Then problem \eqref{deter_lower_upper} with this supply as the lower bound can always give a integer solution.  
