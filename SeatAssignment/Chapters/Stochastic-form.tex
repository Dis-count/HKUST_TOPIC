...
Consider the decision makers who give the seat assignment based on the scenarios then assign the groups to seats according to the realized true demand. For example, When there is no seating for group of four, seats of 5 can be assigned to a group of 4 with one empty seat as social distance.
...
\begin{algorithm}[H]
  \caption{Feasible seat assignment algorithm}\label{feasible_seat}
    ...
    \item[Step 2.] Aggregate the solution to the supply, $\mathbf{s}^{0} = \sum_{j} x^{*}_{ij}$.

    \item[Step 3.] Set $\mathbf{s}^{0}$ as the upper bound of demand in problem \eqref{deter_upper}. Obtain the optimal solution $\mathbf{x}^{1}$.
    
    \item[Step 4.] Aggregate the solution to the supply, $\mathbf{s}^{1} = \sum_{j} x^{1}_{ij}$.

    \item[Step 5.] Obtain the solution, $\mathbf{x}^{2}$, from \eqref{deter_lower} by setting the supply $\mathbf{s}^{1}$ as the lower bound. 
    \item[Step 6.] Aggregate the solution to the supply, $\mathbf{s}^{2} = \sum_{j} x^{2}_{ij}$, which is the feasible seat assignment.
   \end{description}
  \end{algorithm}
