...
\begin{equation}\label{appro_dp}
    v^{t}(\bm{L}) = \theta^{t} + \sum_{j=1}^{N} \max_{\bm{h} \in S(L_{j})} \{\sum_{i=1}^{M} \beta_{ij} h_{i}\}.
\end{equation}
...
\begin{align}
    \theta^{t} - \theta^{t+1} = v^{t}(\bm{L}) - v^{t+1}(\bm{L}) \geq  \sum_{i} p_{i} \max\left\{\max_{j:\bm{h} \in S(L_{j}), h_{i} \geqslant 1}\{v^{t+1}(\bm{L} - e_{j}^{T} n_{i}) - v^{t+1}(\bm{L}) + i\}, 0\right\}
\end{align}
...
Then, $\theta^{1} = \sum_{t=1}^{T} (\theta^{t} - \theta^{t+1}) \geq \sum_{t} \sum_{i} \alpha_{i} p_{i} = \sum_{i} d_{i} \alpha_{i}$. $v^{1}(\bm{L}) = \sum_{i} d_{i} \alpha_{i} + \sum_{j} \gamma_{j}$.
...
$V(\text{T-BPC})$ denotes the expected optimal value of \eqref{bid-price_dual}.
 
$V(\text{I-BPC})$ denotes the expected optimal value of \eqref{improve_bid}.

$V(\text{OPT})$ denotes the expected value of the optimal offline policy.
...
established via a ``dual'' formulation, which lose the information contained in the primal
problem.
