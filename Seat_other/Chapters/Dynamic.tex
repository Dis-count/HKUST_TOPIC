...
Let $val(\text{T-BPC})$, $val(\text{DLP})$ denote the optimal value of \eqref{bid-price_dual} and the LP relaxation of the \textup{SPDR} problem with expected demand, respectively. Then we have $val(\text{T-BPC}) = val(\text{DLP})$.
...
There are some drawbacks on the traditional bid-price control. First, it cannot distinguish difference between rows, when the capacity allows, placing any row is the same for this pocily; second, it may not be feasible to accpet the request by the decision when the capacity of some row doesnot allow.

To address this problem, we consider an improved BPC policy. The basic idea is to represent the structure of rows by patterns, then use the patterns to make the decision.
...
$\alpha_{i}$ represents marginal revenue for group $i$. $\beta_{ij}$ represents the cost for group $i$ assigned in row $j$. $\gamma_{j}$ represents the capacity cost for row $j$.
...
Let $val(\text{I-BPC})$ denote the expected optimal value of \eqref{improve_bid} with expected demand. Then we have $val(\text{T-BPC}) \geq val(\text{I-BPC}) = val(\text{DLP}) \geq val(\text{HO}) \geq val(\text{OPT})$.

The improved BPC policy approximate the optimal value better than the traditonal BPC policy.

As a comparison with the traditional BPC policy, the improved BPC policy can distinguish the different rows and does need to judge whether the acceptance decision is feasible, once the request is accepted, it can be placed in some row.
...
The dual may loss the information of the remaining demand, we then consider the primal problem.
Let $y_{j \bm{h}}$ denote whether to use pattern $\bm{h}$.

Relax $y_{j \bm{h}}$, we have the primal problem:
...
Efficient patterns: the optimal solution to $\max_{\bm{h}} \sum_{i=1}^{M} [n_i - \delta - \alpha_{i}] h_{i} - \gamma_{j}$.

To generate all efficient patterns, we need to solve multiple subproblems.
...
\begin{lem}
    If $y_{j \bm{h}}^{*} > 0$ is the optimal solution to the \eqref{improve_bid}, then $\bm{h}$ is an efficient pattern.
\end{lem}


The efficient pattern must be a dominating pattern. A pattern $\bm{h}$ is dominant if there is no distinct pattern $\bm{h}{'}$ where every component of $\bm{h}{'}$ is greater than or equal to the corresponding component of $\bm{h}$.
