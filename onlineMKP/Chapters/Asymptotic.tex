...
The second benchmark is the offline relaxed optimal value $V^{\text{HO}}$, which is defined as the optimal value of the same offline problem but with the integrality constraints $x_{ij} \in \mathbb{N}$ relaxed to $x_{ij} \geq 0$. By definition, we have $V^{\text{HO}} \geq V^{\text{OFF}}$. 

Let $V^{\pi}$ denote 


The expected loss of a policy $\pi$ is therefore bounded by
...
The revenue loss between HO and DPP can be decomposed into $T$ increments. Each increment represents the gap between two ``adjacent'' OPTs. The gap can be uniformly bounded by a constant $l$ related to $r_{M}$. For any $\omega$, $OPT^{\omega}(\bm{C}, d^{[1, T]}, \gamma^{[1,t)}) - OPT^{\omega}(\bm{C}, d^{[1, T]}, \gamma^{[1,t+1)}) \leq l$.
...
\stackrel{(\mathrm{a})}{\leqslant} & \mathbb{P}\left(\max_{i^{\prime}}\left|\mathbb{E}\left[d_{i^{\prime}}^{[t, \theta T]}\right]-d_{i^{\prime}}^{[t, \theta T]}\right|> \frac{\mathbb{E}\left[d_{i^t}^{[t, \theta T]}\right]-N-1}{\delta (N+1)} \right) \\
= & \mathbb{P}\left(\bigcup_{i^{\prime}}\left\{\left|\mathbb{E}\left[d_{i^{\prime}}^{[t, \theta T]}\right] - d_{i^{\prime}}^{[t, \theta T]}\right|> \frac{\mathbb{E}\left[d_{i^t}^{[t, \theta T]}\right]-N-1}{\delta (N+1)} \right\}\right) \\
\leqslant & \sum_{i^{\prime}} \mathbb{P}\left(\left|\mathbb{E}\left[d_{i^{\prime}}^{[t, \theta T]}\right]-d_{i^{\prime}}^{[t, \theta T]}\right|> \frac{\mathbb{E}\left[d_{i^t}^{[t, \theta T]}\right]-N-1}{\delta (N+1)}\right) \\
\stackrel{(\mathrm{b})}{=} & \sum_{i^{\prime}} \sum_{i} \mathbb{P}\left(\left.\left|\mathbb{E}\left[d_{i^{\prime}}^{[t, \theta T]}\right]-d_{i^{\prime}}^{[t, \theta T]}\right| > \frac{\mathbb{E}\left[d_{i^t}^{[t, \theta T]}\right]-N-1}{\delta (N+1)} \right\rvert\,\left(i^t = i\right) \right) \mathbb{P}\left(i^t = i \right)  \\
\leqslant & \sum_{i} \sum_{i^{\prime}} \mathbb{P}\left(\left.\left|\mathbb{E}\left[d_{i^{\prime}}^{[t, \theta T]}\right]-d_{i^{\prime}}^{[t, \theta T]}\right| > \frac{\mathbb{E}\left[d_{i^t}^{[t, \theta T]}\right]-N-1}{\delta (N+1)} \right\rvert\,\left(i^t = i\right) \right) \bm{1}\left\{t \leqslant \theta T\right\} \\
\stackrel{(\mathrm{c})}{=} & \sum_{i} \sum_{i^{\prime}} \mathbb{P}\left(\left|\mathbb{E}\left[d_{i^{\prime}}^{[t, \theta T]}\right]-d_{i^{\prime}}^{[t, \theta T]}\right| > \frac{\mathbb{E}\left[d_{i^t}^{[t, \theta T]}\right]-N-1}{\delta (N+1)} \right) \bm{1}\left\{t \leqslant \theta T\right\}.
\end{aligned}
$$
...
Let $T_0=\left\lceil\frac{N+1}{\lambda_{\min}}\right\rceil<\frac{\theta}{2}$.
For $t \leq \theta T - T_{0}$, $\frac{\mathbb{E} [d_{i}^{(t, \theta T]}]- N-1}{\delta (N+1)} \geqslant \frac{\lambda_{\min}\left(\theta T-t\right)- \lambda_{\min} T_0 / 2}{\delta (N+1)} \geqslant \frac{\lambda_{\min}\left(\theta T-t\right)}{2 \delta (N+1)}$.
...
\leqslant & 2 l \sum_{t=1}^{\theta T} \sum_{i} \sum_{i^{\prime}} \mathbb{P}\left(\left|\mathbb{E}\left[d_{i^{\prime}}^{(t, \theta T]}\right]- d_{i^{\prime}}^{(t, \theta T]}\right|>\frac{\mathbb{E}\left[d_{i^{t}}^{[t, \theta T]}\right]- N-1}{\delta (N+1)}\right) \bm{1}\left\{t \leqslant \theta T\right\} \\
\leqslant & 2 l \sum_{i} \sum_{t=1}^{\theta T} \sum_{i^{\prime}} \mathbb{P}\left(\left|\mathbb{E}\left[d_{i^{\prime}}^{(t, \theta T]}\right]-d_{i^{\prime}}^{(t, \theta T]}\right|> \frac{\lambda_{\min}\left(\theta T-t\right)}{2 \delta (N+1)}\right) \\
 \stackrel{(\mathrm{a})}{\leqslant} & 2 l \sum_{i} \sum_{t=1}^{\theta T-T_0} \sum_{i^{\prime}} 2 \exp \left(- 2 \frac{\left(\lambda_{\min}\left(\theta T-t\right)\right)^2}{\left(2 \delta (N+1)\right)^2(\theta T-t)}\right)+ 2 l \sum_{i} \sum_{t=\theta T-T_0+1}^{\theta T} \sum_{i^{\prime}} 1 \\
= & 2 l \sum_{i} \sum_{t=1}^{\theta T-T_0} \sum_{i^{\prime}} 2 \exp \left(- \frac{\lambda_{\min }^2}{2 \delta^2 (N+1)^2} \cdot (\theta T-t)\right)+ O(1) \\
\leqslant & 2 l M \sum_{i} \sum_{t=1}^{\theta T - T_0} 2 \exp \left(- \frac{\lambda_{\min}^2}{2 \delta^2 (N+1)^2} \cdot (\theta T-t)\right)+O(1).
\end{aligned}
$$
...
$$
\begin{aligned}
& 2 l M \sum_{i} \sum_{t=T_0}^{+\infty} 2 \exp \left(- \frac{\lambda_{\min}^2 t^2}{2 \delta^2 (N+1)^2 t}\right) + O(1) \\
= & 2 l M^2 \sum_{t=T_0}^{+\infty} 2 \exp \left(-\left(\frac{\lambda_{\min}^2}{2 \delta^2 (N+1)^2}\right) \cdot t\right)+O(1) \\
= & O(1)
\end{aligned}
$$

The last equality is because for given $C>0$, $\sum_{t=1}^{+\infty} \exp(-Ct) = O(1)$.
