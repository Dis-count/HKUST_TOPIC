...
construct a set $S(c_{j})$ which contains all feasible patterns. For a more general case where $\bm{h}$ is subject to additional constraints, the set $S(c_{j})$ can be extended to include only those solutions that satisfy them. The following algorithm can also be adjusted accordingly.
...
For the BPP, the optimal bid prices are $\bm{\beta}{1}^{*} = [4, 4, 4, 6]$, $\bm{\beta}{2}^{*} = [6, 6, 6, 6]$, and $\bm{\beta}_{3}^{*} = [10, 8, 8, 8]$. The resulting objective value is $z(BPP) = 40$, which is less than $z(BPC) = \frac{124}{3}$. Using type 3 as an example, we see it can only be assigned to knapsack 2 or 3. From the item's perspective, its reduced reward vector $r_{3} - \bm{\beta}_{3}^{*} = [-2, 0, 0, 0]$ reveals it is unprofitable for knapsack 1, while from the knapsack's perspective, the optimal solution requires two patterns, $[0, 1, 0]$ and $[1, 0, 0]$ for knapsack 4, indicating that type 3 is also excluded from this knapsack.
