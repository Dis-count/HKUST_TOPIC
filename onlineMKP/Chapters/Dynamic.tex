...
% $\beta_{ij}^{\dag} = n_{i} \beta_{j}$ is feasible for \eqref{improve_bid}, then the optimal bid-price 
...
% This lemma 与 lemma \ref{BPP} 相比，when $x_{ij}^{*} > 0$, 则一定存在j for $\bm{h}^{*} \in \arg\max_{\bm{h} \in S(c_j)} \sum_{i} \beta_{ij}^{\dag} h_{i}, h_{i}^{*} \geq 1$ 而不需要单独check是否存在。
