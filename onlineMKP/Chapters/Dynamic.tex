...
Then, we obtian $\theta^{1} = \sum_{t=1}^{T} (\theta^{t} - \theta^{t+1}) \geq \sum_{t} \sum_{i} \alpha_{i} p_{i} = \sum_{i} d_{i} \alpha_{i}$ and $\hat{v}^{1}(\bm{C}) = \sum_{i} d_{i} \alpha_{i} + \sum_{j} \gamma_{j}$.
...
\begin{algorithm}[H]
    ...
    \For{$t =1, \ldots, T$}{
      ...
      \eIf{$r_i - \beta_{ij} > 0$ and $\max_{j \in \mathcal{N}}{c_j^{t}} \geq w_i$}
      {Set $k = \arg \max_{j \in \mathcal{N}}\{r_i - \beta_{ij}\} $ and break ties\;
      Assign the item to knapsack $k$, let $c_{k}^{t+1} \gets c_{k}^{t} - w_{i}$\;}
      {Reject the request\;}}
\end{algorithm}
...
% Proof:
% $\sum_{j} x_{ij}^{*} < d_{i}$, then $\alpha_{i}^{*} = 0$. $i - \beta_{ij} \leq 0$.
