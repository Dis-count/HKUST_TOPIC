...
We develop several policies for the DSMKP. Because the canonical bid-price control policy does not distinguish between knapsacks when assigning items, we first develop a pattern-based bid-price control policy. To address its cumbersome feasibility-checking procedure, we then propose a primal dynamic policy, which is straightforward and easily implementable. This policy has an asymptotic loss of 
$O(1)$. We evaluate our approach through a series of numerical experiments.
